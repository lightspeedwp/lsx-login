...
if ( ! class_exists( 'LSX_Login' ) ) {
	...
	class LSX_Login {
		...
		/**
		 * @var object LSX_Login_Emails
		 */
		public $emails;

		/**
		 * @var object LSX_Login_WooCommerce
		 */
		public $woocommerce;
		...
		public function __construct() {
			...
			require_once( LSX_LOGIN_PATH . 'classes/class-lsx-woocommerce.php' );

			$this->emails = LSX_Login_Emails::instance();
			$this->woocommerce = LSX_Login_WooCommerce::instance();
		...
		public function scripts() {
			wp_enqueue_style( 'lsx-login', LSX_LOGIN_URL . 'assets/css/lsx-login.css', array(), LSX_LOGIN_VER );
			wp_style_add_data( 'lsx-login', 'rtl', 'replace' );

			$enqueue_scripts = apply_filters( 'lsx_login_js_enqueue', true );

			if ( true === $enqueue_scripts ) {
				wp_enqueue_script( 'lsx-login', LSX_LOGIN_URL . 'assets/js/lsx-login.min.js', array( 'jquery' ), LSX_LOGIN_VER, true );
				...
				wp_localize_script( 'lsx-login', 'lsx_login_params', $params );
			}
		}
		...
		public function admin_scripts() {
			# wp_enqueue_style( 'lsx-login-admin', LSX_LOGIN_URL . 'assets/css/lsx-login-admin.css', array(), LSX_LOGIN_VER );

			wp_enqueue_script( 'lsx-login-admin', LSX_LOGIN_URL . 'assets/js/lsx-login-admin.min.js', array( 'jquery' ), LSX_LOGIN_VER, true );

			$params = array(
				'text_send'          => __( 'Send Message', 'lsx-login' ),
				'text_sending'       => __( 'Sending...', 'lsx-login' ),
				'text_sent'          => __( 'Done! The emails are being sent', 'lsx-login' ),
				'ajax_welcome_users' => admin_url( 'admin-ajax.php?action=lsx_welcome_users' ),
			);
		...
		/**
		 * Checks the password and returns true or not
		 * @return object | bool
		 */
		...
		public function do_ajax_reset_confirmed() {
			if(isset($_POST['key']) && isset($_POST['login']) && isset($_POST['pass1']) && isset($_POST['pass2']) ){
				...
				if(!is_wp_error($user)){
					$result['success']  = 1;
					$result['message']  = __('Your password has been reset! Please click the login tab to login with your new password.','lsx-login');
		...
		*/
		...
		public function metaboxes( array $meta_boxes ) {
			...
			$fields = array(
				array(
						'id' => 'lock_single',
						'name' => esc_html__( 'Restrict access', 'lsx-login' ),
						'type' => 'checkbox'
				),
				array(
						'id' => 'public_content',
						'name' => esc_html__( 'Public content', 'lsx-login' ),
						'type' => 'wysiwyg'
				),
			);
		...
		public function useful_links( $atts ) {
			...
			$account_slug = $this->woocommerce->get_my_account_page_slug();
