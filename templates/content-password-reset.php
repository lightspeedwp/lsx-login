...
if(isset($_GET['action']) && 'rp' === $_GET['action'] && !is_wp_error($is_key_confirmed)){ ?>
	<form autocomplete="off" method="post" action="<?php home_url('/'); ?>" class="lostpasswordform" name="resetpassform">
		<input type="hidden" autocomplete="off" value="<?php echo esc_attr( isset( $_GET['login'] ) ? sanitize_user( wp_unslash( $_GET['login'] ), true ) : '' ); ?>" class="user_login" name="user_login">
		<input type="hidden" value="<?php echo esc_attr( isset( $_GET['key'] ) ? sanitize_text_field( wp_unslash( $_GET['key'] ) ) : '' ); ?>" name="rp_key">
